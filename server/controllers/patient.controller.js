...
exports.buyPackage = async (req, res) => {
    try {
        ...
        if (firstBill) {
            ...
            const timeLimit =
                package.time_limit.value * conversion[package.time_limit.unit]; //a number (represent days)
            const days = 1000 * 60 * 60 * 24;
            let i = 0;
            while (true) {
                if (
                    timeFirstBuy.getTime() + timeLimit * i * days <=
                    timeBuy.getTime()
                ) {
                    i++;
                } else {
            ...
            const timeLeft = new Date(
                timeFirstBuy.getTime() + timeLimit * (i - 1) * days,
            );
            const timeRight = new Date(
                timeFirstBuy.getTime() + timeLimit * i * days,
            );
