...
function ManagerPage() {
    ...
    let initListManager = async () => {
        try {
            ...
            doctors.forEach((doctor) => {
                if (doctor.account_id !== null){
                    dispatch(clearList());
                    dispatch(addManager(doctor.account_id));
                }
            })
        } catch (err) {
            console.log(err)
    ...
    let formatList = (list) => {
        ...
        let newList = list.map((item, index) => {
            return {
                username: <span className={cx({
                    inactive: item.status === 'inactive',
                })}>{item.username}</span>,
                ig1: '',
                ig2: '',
                history: <Link className={cx('btn-item', {
                    inactive: item.status === 'inactive',
                })} to="">view history</Link>,
                inactive: <div onClick={() => handleInactive(index)} className={cx('btn-item')}>inactive</div>,
            }
        })
