...
function Payment() {
    ...
    let changeCredit = useCallback(
        async (e) => {
            ...
            if (res.message && res.message === 'Credit limit updated successfully') {
                getCurCredit();
                dispatch(setMess({ mess: 'Credit limit updated successfully', type: 'success' }));
            }
        },
        [valCredit],
    );
    ...
    let handleMakeNoti = useCallback(async (id_number) => {
        ...
        let res = await postAPI(
            'doctor/patients/debt-notification',
            {
                id_number: id_number,
            },
            token,
        ...
        console.log(res);
        dispatch(setMess({ mess: 'Notification sent successfully', type: 'success' }));
    }, []);
    ...
    let handleSendNotiAll = useCallback(async () => {
        ...
        if (tmp.length > 0) {
            let res = await postAPI(
                'doctor/patients/debt-notification-all',
                {
                    id_numbers: tmp,
                },
                token,
            ...
            <WrapContent>
                ...
                <div className={cx('wrap-list')}>
                    <div className={cx('flex-center', 'space-between')}>
                        ...
                        <button onClick={handleSendNotiAll} className={cx('noti-btn')}>
                            send notification to all patients in debt
                        </button>
                    </div>
                    <ListItem
                        noUnderLine
                    ...
                    {listPayAcc.map((item, index) => {
                        let nForm = {
                            ...
                            debt: item.in_debt + '',
                            btn: item.in_debt && (
                                <button onClick={() => handleMakeNoti(item.id_number)} className={cx('noti-btn')}>
                                    send notification
                                </button>
                            ),
                        };
                        return <ListItem key={index} infos={nForm} />;
                    })}
                </div>
            </WrapContent>
        </div>
