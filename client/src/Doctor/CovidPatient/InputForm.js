...
function InputForm() {
  ...
  let validateForm = useCallback((inputField, selectValue) => {
    ...
    if (inputField.Year_of_birth !== '') {
      let year = Number(inputField.Year_of_birth.split('-')[0]);
      let now = new Date();
  ...
  let handleSubmit = useCallback(() => {
    ...
    if (readySubmit) {
      ...
      let dataSubmit = {
        ...
        address: `${selectValue.Province} ${selectValue.District === '-- make your choice --' ? '' : selectValue.District
        ...
        <WrapContent>
          ...
          <div className={cx('row', 'field-input')}>
            {inputField1.map((title, index) => {
              ...
              let type = formatedTitle === 'Year_of_birth' || formatedTitle === 'Name' || formatedTitle === 'Password' ? 'text' : 'number';
              return (
